...
//$_SESSION['tmptxt'];
//$_SESSION['usuario']="";
if (isset($_POST['usuario']) && isset($_POST['contra']) && isset($_POST['reresponse'])){
    $usu=$_POST['usuario'];
    $contra=$_POST['contra'];
    $captcha=$_POST['reresponse'];
    include("model/class_usuarios_dal.php");
    $obj_usuario=new usuarios_dal;
    $existe=$obj_usuario->verify_user($usu,$contra);
    //echo 'logn acciones:'.$existe.'-'.strlen($captcha);exit;
    if (($existe==1) and (strlen($captcha)!=0)) {
        //session_start();
